...
const About = ({ data, pageContext }) => {
	...
	let scrollSpyItems = sections && sections.map(section => section.title.toLowerCase());;

	return (
		<Layout>
			<div className="row mt-6">
				<div className="col-12 d-none d-lg-block col-lg-5 col-xl-6 about-nav">
					{ scrollSpyItems && <Scrollspy items={ scrollSpyItems } currentClassName="active">
						{sections.map(section => (
							<li key={`about-${section.title}`}><HollowLink link={`#${section.title.toLowerCase()}`} text={section.title} /></li>
						))}
					</Scrollspy>
					}
				</div>
				<div className="col-12 col-lg-7 col-xl-6">
					{sections && sections.map(section => (
						<Row classes="mb-5 pt-4" id={section.title.toLowerCase()} key={`about-section-${section.title}`}>
							<h2 className="d-lg-none d-block about-title">{section.title}</h2>
