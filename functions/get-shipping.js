...
exports.handler = async (event, context) => {
	...
	if (shippingZones.data) {
		...
		let found = false;
		await Promise.all(shippingZones.data.map(async (zone) => {
			if(found){
				return;
			}
			const countries = await api.get(`shipping/zones/${zone.id}/locations`);
			if(countries && countries.data && countries.data.length){
				if(countries.data.find(country => country.code === targetCountry)){
